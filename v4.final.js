...
Dom.on('login', 'submit', function (ev) {
  ...
  setTimeout(() => {
    ...
    networkManager.join(
      roomId,
      name,
      Util.randomInt(0, SPRITES.CARS.length - 1)
    );
...
function update(dt) {
  ...
  if (
    networkManager.raceState === RACE_STATE.COUNTDOWN &&
    (finished || lapCount > 0)
  ) {
    lapCount = 0;
    totalTime = 0;
  ...
  if (
    networkManager.raceState === RACE_STATE.WAITING ||
    networkManager.raceState === RACE_STATE.COUNTDOWN ||
...
function updateCars(dt, playerSegment, playerW) {
  ...
  for (n = 0; n < cars.length; n++) {
    ...
    car.checkTraffic(segments, segmentLength, {
      x: playerX,
      w: playerW,
      speed: speed,
      z: position + playerZ
  ...
  for (n = drawDistance - 1; n > 0; n--) {
    ...
    for (const id in networkManager.remotePlayers) {
      ...
      if (rSegment === segment) {
        ...
        var rScale = Util.interpolate(
          segment.p1.screen.scale,
          segment.p2.screen.scale,
          rPercent
        );
        var rX =
          Util.interpolate(segment.p1.screen.x, segment.p2.screen.x, rPercent) +
          (rScale * player.x * roadWidth * width) / 2;
        var rY = Util.interpolate(
          segment.p1.screen.y,
          segment.p2.screen.y,
          rPercent
        );
        ...
        var spriteH =
          ((player.sprite.h * rScale * width) / 2) *
          (SPRITES.SCALE * roadWidth);
        var nameY = rY - spriteH;
        if (!segment.clip || nameY > segment.clip) {
          // Simple clipping check (mostly correct)
...
function resetSprites() {
  ...
  for (n = 200; n < segments.length; n += 3) {
    addSprite(
      ...
      Util.randomChoice([1, -1]) * (2 + Util.random() * 5)
    );
  }
...
function resetCars() {
  ...
  for (n = 0; n < totalCars; n++) {
    ...
    speed =
      maxSpeed / 4 +
      (Util.random() * maxSpeed) / (sprite == SPRITES.SEMI ? 4 : 2);
...
function reset(options) {
  ...
  if (options.simulatedLatency !== undefined)
    networkManager.simulatedLatency = options.simulatedLatency;
  if (options.smoothing !== undefined)
    networkManager.smoothing = options.smoothing;
