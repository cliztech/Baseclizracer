...
export function setupTweakUI(
  callbacks,
  { Dom = defaultDom, Util = defaultUtil } = {}
) {
  ...
  Dom.on('roadWidth', 'input', function (ev) {
    reset({
  ...
  Dom.on('cameraHeight', 'input', function (ev) {
    reset({
  ...
  Dom.on('drawDistance', 'input', function (ev) {
    reset({
  ...
  Dom.on('fieldOfView', 'input', function (ev) {
    reset({
  ...
  Dom.on('fogDensity', 'input', function (ev) {
    reset({
  ...
  Dom.on('simulatedLatency', 'input', function (ev) {
    reset({
      simulatedLatency: Util.limit(
        Util.toInt(ev.target.value),
        Util.toInt(ev.target.getAttribute('min')),
        Util.toInt(ev.target.getAttribute('max'))
      )
  ...
  Dom.on('smoothing', 'input', function (ev) {
    reset({
      smoothing: Util.limit(
        Util.toInt(ev.target.value),
        Util.toInt(ev.target.getAttribute('min')),
        Util.toInt(ev.target.getAttribute('max'))
      )
...
export function refreshTweakUI(state, { Dom = defaultDom } = {}) {
  ...
  if (Dom.get('currentSimulatedLatency')) {
    Dom.get('currentSimulatedLatency').innerHTML = Dom.get(
      'simulatedLatency'
    ).value = simulatedLatency;
    Dom.get('currentSmoothing').innerHTML = Dom.get('smoothing').value =
      smoothing;
  }
}
