...
async function test() {
  ...
  assert.strictEqual(
    room.state,
    RACE_STATE.WAITING,
    'Should be WAITING with 1 player'
  );
  ...
  assert.strictEqual(
    ...
    'Should be COUNTDOWN with 2 players'
  );
  ...
  assert.strictEqual(
    room.state,
    RACE_STATE.RACING,
    'Should be RACING after timeout'
  );
  ...
  assert.strictEqual(
    updates.length,
    2,
    'Should receive 2 state updates (Countdown, Racing)'
  );
  ...
  assert.strictEqual(
    room.state,
    RACE_STATE.WAITING,
    'Should revert to WAITING when player leaves'
  );
